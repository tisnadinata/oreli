    ...
    "use strict";
	 ...
	 $j('.ajax-to-cart').click(function(e){
		e.preventDefault();
		 ...
		 var kode = $(this).val();		 
		  var dataString = 'addtocart='+kode;
		 $j.ajax({
			 type :"post",
			 url: "http://localhost/oreli/ajax.php",
			 data:dataString,
	 ...
	 $j('#btn-to-cart').click(function(e){
		e.preventDefault();
		  var kode = $('#kode_produk').val();		 
		  var ukuran = $('input:radio[name=ukuran]:checked').val();		 
		  var warna = $('input:radio[name=warna]:checked').val();		 
		  var qty = $('#qty').val();
		  var dataString = 'toCart='+kode+"/"+ukuran+"/"+warna+"/"+qty;
			$j.ajax({
				 type :"post",
				 url: "http://localhost/oreli/ajax.php",
				 data:dataString,
				 success: function(response){
					$j('#btn-to-cart').removeClass('btn--wait');
					$j('#modalAddToCart').modal("toggle");				
				}
			});				  
	});
	 ...
	 $j('.ajax-to-wishlist').click(function(e){
		...
		var dataString = 'addtowishlist='+kode;
		 $j.ajax({
			 type :"post",
			 url: "http://localhost/oreli/ajax.php",
			 data:dataString,
...
jQuery(function($j) {
	...
	if($j('#subscribe-form').length){
		$j('#subscribe-form').validate({
			...
			submitHandler: function(form) {
			var email = $j('.subscribe-form__input').val();
			var dataString = "subscribe="+email;
				$j.ajax({
					type:"POST",
					data: dataString,
					url:"http://localhost/oreli/ajax.php",
					success: function() {
						  alert("Selamat, email anda sudah kami simpan dan akan mendapat informasi terbaru dari kami.");
