...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'enigma.beon.co.id;101.50.1.15';  // Masukkan Server SMTP
  $mail->Port 		  = 465;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'no-reply@oreli.co.id';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'xGNcfkL3[fVl';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'no-reply@oreli.co.id';        // Masukkan default from pada email
  $default_email_from_name  = 'PT. Toko Ritel Indonesia';           // Masukkan default nama dari from pada email
